...
"""fasdoc version 0.1, 2011-06-14."""

# Author: Fasih Ahmad Fakhri (fasihahmadfakhri@gmail.com).
# Clone of jemdoc version 0.7.1, 2011-06-14.
# Copyright (C) 2007-2011 Jacob Mattingley (jacobm@stanford.edu).
#
# This file is part of fasdoc.
#
# fasdoc is free software; you can redistribute it and/or modify it under the
# terms of the GNU General Public License as published by the Free Software
# Foundation; either version 3 of the License, or (at your option) any later
# version.
#
# fasdoc is distributed in the hope that it will be useful, but WITHOUT ANY
...
def showhelp():
  a = """Usage: fasdoc [OPTIONS] [SOURCEFILE] 
  Produces html markup from a fasdoc SOURCEFILE.

  Most of the time you can use fasdoc without any additional flags.
  For example, typing

    fasdoc index

  will produce an index.html from index.fasdoc, using a default
  configuration.
  ...
  for example,
    ...
    fasdoc -c mywebsite.conf index.fasdoc 
...
def standardconf():
  ...
  [defaultcss]
  <link rel="stylesheet" href="fasdoc.css" type="text/css" />
  ...
  while 1: # wait for EOF.
    ...
    # look for blocks.
      ...
      if infoblock:
        ...
        # process fasdoc markup in titles.
        if len(g) >= 1:
          g[0] = br(g[0], f)
...
def main():
  ...
  for j in range(i, len(sys.argv)):
    # First, if not a file and no dot, try opening .fasdoc. Otherwise, fall back
  ...
  for inname in innames:
    if outname is None:
      thisout = re.sub(r'.fasdoc$', '', inname) + '.html'
    elif os.path.isdir(outname):
      # if directory, prepend directory to automatically generated name.
      thisout = outname + re.sub(r'.fasdoc$', '', inname) + '.html'
    else:
      thisout = outname
